...
const Stopwatch = () => {
    ...
    const getHours = value => {
        return Math.floor(value / 360000).toString().padStart(2, '0')
    }
    const getMinutes = value => {
        return Math.floor((value % 360000) / 6000).toString().padStart(2, '0')
    }
    const getSeconds = value => {
        return Math.floor((value % 6000) / 100).toString().padStart(2, '0')
    }
            ...
            return (
                <li className="item" key={item + index}>
                    <span className="item-result">
                        ...
                        {getMinutes(item)}:
                        {getSeconds(item)}:
                        {getMilliseconds(item)}
                    </span>
                    <button className="btnDelete" onClick={() => deleteData(index)}>delete</button>
                </li>
            )
        })}
